...
//        SetupTween:
		...
		public void AddListener(string type, Action callback)
		{
			UnityEvent ue = new UnityEvent();
			ue.AddListener(() => {callback();});
			...
			case "update":
				if (tween == null) {
					...
					tween.OnUpdate(ue.Invoke);
				}
                break;
			case "stepComplete":
				if (tween == null) {
					hasOnStepComplete = true;
					onStepComplete = ue;
				}else{
					tween.OnStepComplete(ue.Invoke);
				}
                break;
			case "complete":
				if (tween == null) {
        ...
        public override void DORestart(bool fromHere = false)
        {
        	_playCount = -1;
            if (tween == null) {
				if (DG.Tweening.Core.Debugger.logPriority > 1) DG.Tweening.Core.Debugger.LogNullTween(tween); return;
            }
